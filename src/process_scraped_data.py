...
def load_ingredients_lists(path):
  with open(path, 'r') as f:
    ...
    def alphanum(x): return re.sub("[^a-zA-Z]", " ", x).strip().lower()
...
def main():
  for root, dirname, filenames in os.walk('../dat/RecipesScraper/output'):
    for filename in filenames:
      name, ext = os.path.splitext(filename)
      out_path = os.path.join('../dat/processed', name + '.csv')
