...
class JamieOliverScraper(scrapy.Spider):
    ...
    def parse_recipe(self, response):
        ...
        recipe_name = self.remove_non_ascii(response.css("h1.hidden-xs::text").extract_first())
        ingredients_list = response.css("ul.ingred-list")
        ingredients = [' '.join(ingredient.strip().split()).replace(' , ', ', ')
                       for ingredient in ingredients_list.css("li::text").extract()]
        url_tags = [u" ".join(tag.split("-")).capitalize() for tag in response.url.split("/")[4:-2]]
