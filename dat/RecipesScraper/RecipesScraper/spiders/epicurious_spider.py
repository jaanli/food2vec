...
class Epicurious(scrapy.Spider):
    ...
    editorial_recipes = ["http://www.epicurious.com/services/sitemap/recipes/editorial/{}?page=1".format(year)
                  for year in range(1998, 2018)]
    member_recipes = ["http://www.epicurious.com/services/sitemap/recipes/member/{}?page=1".format(year)
                  for year in range(2005, 2018)]
    start_urls = editorial_recipes + member_recipes

    def parse(self, response):
        """Parse the recipe list."""
        recipes = LinkExtractor(allow=("/recipes/.*/views")).extract_links(response)
        if len(recipes) > 0:
            for recipe_link in recipes:
                yield scrapy.Request(recipe_link.url, callback=self.parse_item)

            base_url, page = response.url.split("=")
            yield scrapy.Request("{}={}".format(base_url, int(page) + 1), callback=self.parse)
        else:
    ...
    def parse_item(self, response):
        ...
        if schema_type == "mde":
            ...
            recipe_output_item['ingredients'] = [ingredient for ingredient in recipe['ingredients'] if ingredient not in ['', 'Add all ingredients to list']]
            ...
            recipe_output_item['tags'] = [tag['properties']['title'] for tag in data['items'][1:]]
            try:
                recipe_output_item['description'] = recipe['description']
            except KeyError:
                recipe_output_item['description'] = None
